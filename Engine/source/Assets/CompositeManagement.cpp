...
int CKLBCompositeAsset::readString(const unsigned char* stringVal, size_t stringLen, int /*cte_pool*/)
{
	...
	case CLASS_FIELD:
		...
		} else if (strncmp("checkbox", (const char*)stringVal, (sizeof("checkbox") - 1)) == 0) {
			m_pCurrInnerDef->classID = CHECK_CLASSID;
		} else if (strncmp("container", (const char*)stringVal, (sizeof("container") - 1)) == 0) {
			m_pCurrInnerDef->classID = CONTAINER_CLASSID;
		} else if (strncmp("label", (const char*)stringVal, (sizeof("label") - 1)) == 0) {
			m_pCurrInnerDef->classID = LABEL_CLASSID;
		} else if (strncmp("animnode", (const char*)stringVal, (sizeof("animnode") - 1)) == 0) {
			m_pCurrInnerDef->classID = ANIMNODE_CLASSID;
		} else if (strncmp("textbox", (const char*)stringVal, (sizeof("textbox") - 1)) == 0) {
			m_pCurrInnerDef->classID = TEXTBOX_CLASSID;
		} else if (strncmp("task_webview", (const char*)stringVal, (sizeof("task_webview") - 1)) == 0) {
			m_pCurrInnerDef->classID = WEBVIEW_CLASSID;
		} else if (strncmp("task_virtualdoc", (const char*)stringVal, (sizeof("task_virtualdoc") - 1)) == 0) {
			m_pCurrInnerDef->classID = VIRTUALDOC_CLASSID;
		} else if (strncmp("task_score", (const char*)stringVal, (sizeof("task_score") - 1)) == 0) {
			m_pCurrInnerDef->classID = SCORE_CLASSID;
		} else if (strncmp("task_progressbar", (const char*)stringVal, (sizeof("task_progressbar") - 1)) == 0) {
			m_pCurrInnerDef->classID = PROGRESSBAR_CLASSID;
		} else if (strncmp("task_group", (const char*)stringVal, (sizeof("task_group") - 1)) == 0) {
			m_pCurrInnerDef->classID = GROUP_CLASSID;
		} else if (strncmp("task_dragicon", (const char*)stringVal, (sizeof("task_dragicon") - 1)) == 0) {
			m_pCurrInnerDef->classID = DRAGICON_CLASSID;
		} else if (strncmp("task_list", (const char*)stringVal, (sizeof("task_list") - 1)) == 0) {
			m_pCurrInnerDef->classID = LIST_CLASSID;
		} else if (strncmp("task_piechart", (const char*)stringVal, (sizeof("task_piechart") - 1)) == 0) {
			m_pCurrInnerDef->classID = PIECHART_CLASSID;
		} else if (strncmp("task_canvas", (const char*)stringVal, (sizeof("task_canvas") - 1)) == 0) {
			m_pCurrInnerDef->classID = CANVAS_CLASSID;
		} else if (strncmp("task_scrollbar", (const char*)stringVal, (sizeof("task_scrollbar") - 1)) == 0) {
			m_pCurrInnerDef->classID = SCROLLBAR_CLASSID;
		} else if (strncmp("task_scale9", (const char*)stringVal, (sizeof("task_scale9") - 1)) == 0) {
			m_pCurrInnerDef->classID = SCALE9_CLASSID;
		} else if (strncmp("task_varitem", (const char*)stringVal, (sizeof("task_varitem") - 1)) == 0) {
			m_pCurrInnerDef->classID = VARITEM_CLASSID;
		} else if (strncmp("task_tiledcanvas", (const char*)stringVal, (sizeof("task_tiledcanvas") - 1)) == 0) {
